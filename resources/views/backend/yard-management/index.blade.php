                            ...
                            <tbody>
                                @forelse ($data as $d)
                                    <tr>
                                        ...
                                        <td>{{ $d->designation }}</td>
                                         <td>
                                            <img width="60px" src="{{asset('uploads/yardManagement/'.$d->image)}}" alt="Management">
                                        </td>
                                         
                                        <td class="white-space-nowrap">
                                            <div class="btn-group" role="group">
                                                <a href="{{route('yard.edit',encryptor('encrypt',$d->id))}}" class="btn btn-warning me-2 mb-2">Edit</a>
                                                <form action="{{ route('yard.destroy',encryptor('encrypt',$d->id)) }}" method="post" onsubmit="return confirm('Are you sure you want to delete this item?');">
                                                    @csrf
                                                    @method('DELETE')
