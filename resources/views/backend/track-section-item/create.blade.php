...
<div class="page-heading">
    ...
    <div class="page-content">
        <section class="section">
            <div class="card">
                <div class="card-body">
                    <form id="trackSectionItemForm" action="{{ route('track-section-item.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            ...
                            <select name="track_section_id" id="track_section_id" class="form-control" required>
                                @foreach ($sections as $section)
                                    <option value="{{ $section->id }}" {{ (old('track_section_id', $sectionId) == $section->id) ? 'selected' : '' }}>
                                        {{ $section->title }}
                                    </option>
                                @endforeach
                            </select>
                            @if ($errors->has('track_section_id'))
                                <span class="text-danger">{{ $errors->first('track_section_id') }}</span>
                            @endif
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="short_description" id="short_description" class="form-control" cols="30" rows="8">{{ old('short_description') }}</textarea>
